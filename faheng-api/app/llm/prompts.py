...
## 要求
1. 输出 Markdown，层级只用三级：一级标题为合同名称，二级标题（##）为"第X条"条款，三级标题（###）为条款内的子项"X.Y"
2. 子项规则：仅当条款内容确实需要分点（如付款阶段、验收步骤、多类义务）时才用三级标题"X.Y"（如 3.1、3.2），每个子项下再写正文；简单条款直接用段落或列表，不要为了分层而分层
3. 结构完整：鉴于条款、定义、标的与范围、价款与支付、交付与验收、双方权利义务、违约责任、保密、知识产权、不可抗力、争议解决、其他约定、签署栏
4. 不确定的信息用占位符，如【金额】【日期】
5. 立场均衡但默认保护甲方（采购方）利益
6. 不要使用 Markdown 表格
...
请输出修改后的完整合同。保持原有结构与编号体系（一级=合同名，二级=第X条，三级=X.Y 子项），只改动需要改动的部分，其余条款原文保留。不要使用 Markdown 表格。
