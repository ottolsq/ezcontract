...
from app.schemas.draft import ContentIn, DraftGenerateIn, ReviseIn
...
async def export(draft_id: str):
    session = get_draft(draft_id)
    if session is None:
        raise HTTPException(404, "草稿不存在")
    import asyncio

    path = await asyncio.to_thread(draft_service.export_draft_docx, session)
